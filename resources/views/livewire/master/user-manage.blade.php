<div class="p-6 bg-gray-50 min-h-screen">
    ...
    <div class="mb-6 flex gap-3">
        <button wire:click="openUserModal"
            class="px-6 py-3 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition shadow-sm hover:cursor-pointer duration-75">
            + Tambah User & Employee Baru
        </button>
        <a href="{{ route('master-data.import-employee-user.index') }}"
            class="px-6 py-3 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition shadow-sm hover:cursor-pointer duration-75">
            + Tambah via Excel
        </a>
    </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($userBranches as $ub)
                        <tr class="hover:bg-gray-50 transition border border-red-800">
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 border border-blue-800">
                                <button wire:click="toggleActive({{ $ub->id }})"
                                    class="px-3 py-1 rounded-full text-xs font-medium transition {{ $ub->is_active ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }} hover:cursor-pointer duration-75">
                                    {{ $ub->is_active ? 'Active' : 'Inactive' }}
                                </button>
                            </td>
                            <td class="px-6 py-4 flex gap-2 items-center border border-blue-800">
                                {{-- <button wire:click="editAssignment({{ $ub }})"
                                    class="text-blue-600 hover:text-blue-800 text-sm font-medium hover:cursor-pointer duration-75">
                                    edit
                                </button> --}}
                                <button wire:click="editAssignment({{ $ub->id }})"
                                    class="text-blue-600 hover:text-blue-800 text-sm font-medium hover:cursor-pointer duration-75">
                                    edit
                                </button>
                                <button wire:click="deleteAssignment({{ $ub->id }})"
                                    wire:confirm="Yakin hapus assignment ini?"
                                    class="text-red-600 hover:text-red-800 text-sm font-medium hover:cursor-pointer duration-75">
                                    delete
                                </button>
                            </td>
                        </tr>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <button wire:click="$toggle('showUnassignedUsers')" type="button"
                ...
                <div class="flex items-center">
                    ...
                    <span class="font-medium text-gray-700">Users Belum Ter-assign
                        ({{ $unassignedUsers->count() }})</span>
                </div>
                                ...
                                @forelse($unassignedUsers as $user)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-3">
                                            @if ($user->employee)
                                                <span
                                                    class="text-sm text-gray-600">{{ $user->employee->full_name }}</span>
                                            @else
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <button wire:click="$toggle('showUnassignedEmployees')" type="button"
                ...
                <div class="flex items-center">
                    ...
                    <span class="font-medium text-gray-700">Employees Belum Punya User
                        ({{ $unassignedEmployees->count() }})</span>
                </div>
                        ...
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse($unassignedEmployees as $employee)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-3 text-gray-900 font-mono text-sm">
                                            {{ $employee->employee_code }}
                                        </td>
                                        <td class="px-6 py-3 text-gray-900">{{ $employee->full_name }}</td>
                                        <td class="px-6 py-3 text-gray-600 text-sm">{{ $employee->position ?? '-' }}
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
