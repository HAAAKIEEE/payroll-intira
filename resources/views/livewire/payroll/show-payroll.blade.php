<div class="max-w-4xl mx-auto bg-white p-8 shadow-lg border border-gray-300 rounded-lg">
    ...
    <table class="w-full text-sm">
        <tr class="border-b border-gray-200">
            ...
            <td class="text-right py-2 text-gray-800">{{ number_format($payroll->gaji_pokok, 0, ',', '.') }}</td>
        </tr>

        <tr class="border-b border-gray-200">
            <td class="py-2 font-semibold text-gray-700">TRANSPORT ({{ $payroll->hari_kerja }} HARI)</td>
            <td class="text-right py-2 text-gray-800">{{ number_format($payroll->transportasi, 0, ',', '.') }}</td>
        </tr>

        <tr class="border-b border-gray-200">
            <td class="py-2 font-semibold text-gray-700">MAKAN ({{ $payroll->hari_kerja }} HARI)</td>
            <td class="text-right py-2 text-gray-800">{{ number_format($payroll->makan, 0, ',', '.') }}</td>
        </tr>

        <tr class="bg-green-100 border-b border-green-200">
            <td class="py-2 font-semibold text-green-800">BONUS REVENUE</td>
            <td class="text-right py-2 font-semibold text-green-800">{{ number_format($payroll->bonus_revenue, 0, ',', '.') }}</td>
        </tr>

        <tr class="border-b border-gray-200">
            <td class="py-2 font-semibold text-gray-700">TUNJANGAN</td>
            <td class="text-right py-2 text-gray-800">{{ number_format($payroll->tunjangan, 0, ',', '.') }}</td>
        </tr>

        <tr class="text-red-600 border-b border-gray-200">
            <td class="py-2 font-semibold">POTONGAN</td>
            <td class="text-right py-2 font-semibold">-{{ number_format($payroll->potongan, 0, ',', '.') }}</td>
        </tr>

        <tr class="border-b border-gray-200">
            <td class="py-2 font-semibold text-gray-700">SIMPANAN</td>
            <td class="text-right py-2 text-gray-800">-{{ number_format($payroll->simpanan ?? 0, 0, ',', '.') }}</td>
        </tr>

        <tr class="border-t-2 border-gray-400 font-bold text-lg bg-gray-50">
            <td class="py-3 text-gray-800">GRAND TOTAL</td>
            <td class="text-right py-3 text-gray-800">{{ number_format($payroll->total, 0, ',', '.') }}</td>
        </tr>

        <tr class="bg-yellow-300 font-bold text-lg border-2 border-yellow-400">
            <td class="py-3 text-gray-900">TAKE HOME PAY</td>
            <td class="text-right py-3 text-gray-900">{{ number_format($payroll->total, 0, ',', '.') }}</td>
        </tr>
    </table>
